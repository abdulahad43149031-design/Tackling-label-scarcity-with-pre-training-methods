...
Supported variants:
  ViT-Small/16 : depth=8,  heads=8,  d_model=512,  mlp_dim=2048  (~22M params)
  ViT-Base/16  : depth=12, heads=12, d_model=768,  mlp_dim=3072  (~86M params)
  ViT-Large/16 : depth=24, heads=16, d_model=1024, mlp_dim=4096  (~307M params)

Designed for STL-10 (96×96 RGB, 10 classes, patch_size=16 → 36 patches).
ViT-Small/16 is the default for the scratch baseline and as the backbone
architecture for MAE and DINO encoders.
"""
...
class VisionTransformer(nn.Module):
    ...
    Args:
        variant    : One of the keys in VARIANTS (default "ViT-Small/16").
        img_size   : Input image resolution (default 96 for STL-10).
        patch_size : Patch size in pixels (default 16 → 36 patches at 96×96).
        in_channels: Number of input channels (default 3 for RGB).
        num_classes: Number of output classes (default 10 for STL-10).
        drop       : Dropout probability applied throughout.
    """
    ...
    def __init__(
        ...
        # --- Weight init (paper: truncated normal, std=0.02) ---
        self._init_weights()
